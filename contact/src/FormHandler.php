...
class FormHandler
{
	...
	public function __construct()
	{
        ...
        $from_email ='forms@'.$host;
   		$this->mailer->setFrom($from_email,'Fromulaire de contact',false);  
	...
	private function validate_captcha($post)
	{
		...
		if(empty($post['captcha']))
		{
			return json_encode([
						...
						'errors'=>['captcha'=>' Veuillez entrer le code du captcha.']
						]);
		}
		else
		{
			...
			if($_SESSION['user_phrase'] !== $usercaptcha)
			{
				return json_encode([
						'result'=>'captcha_error',
						'errors'=>['captcha'=>' Le code entré ne correspond pas à celui du captcha']
						]);		
			}
		}
		return true;
